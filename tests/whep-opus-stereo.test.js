...
const publisherPath = path.join(__dirname, "..", "tools", "obs-aom-whip-publisher.go");
const publisherSource = fs.readFileSync(publisherPath, "utf8");
assert.match(
  publisherSource,
  /SDPFmtpLine:\s*"minptime=10;useinbandfec=1;stereo=1;sprop-stereo=1"/,
  "the real-media regression publisher must negotiate stereo Opus"
);
const begin = source.indexOf("// BEGIN V1.35 WHEP OPUS STEREO");
const end = source.indexOf("// END V1.35 WHEP OPUS STEREO");
...
console.log("V1.35 WHEP Opus stereo SDP regression tests: PASS");
